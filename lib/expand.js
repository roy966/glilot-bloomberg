...
export function tweetHandle(tweet) {
  return String(tweet?.author?.userName || tweet?.author?.username || "")
    .replace(/^@/, "")
    .toLowerCase();
}

export function replyToHandle(tweet) {
  const raw =
...
/** True when this status is a self-thread reply (same handle as parent). */
export function isSelfReply(tweet, parent = null) {
  if (!needsParent(tweet)) return false;
  const author = tweetHandle(tweet);
  const named = replyToHandle(tweet);
  if (author && named) return author === named;
  if (parent) {
    const ph = tweetHandle(parent);
    return Boolean(author && ph && author === ph);
  }
  return false;
}
...
export async function expandTweet(tweet, { fetchImpl = fetch, getById, collectMedia } = {}) {
  ...
  if (needsParent(tweet)) {
    ...
    // Self-threads KEEP after expanding the parent. Replies to other users
    // DROP if that parent cannot be expanded.
