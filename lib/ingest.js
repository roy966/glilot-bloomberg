...
import { packBatches, lookbackPlan, MAX_HANDLES } from "./query.js";
...
import { ingestSubstack } from "./substack.js";

const ROOT = join(dirname(fileURLToPath(import.meta.url)), "..");
const STEADY_PAGES_PER_BATCH = 2;
const CATCHUP_PAGES_PER_BATCH = 10;
const MAX_FEED = 250;
...
function lastIngestFromLive(rows, stateLast) {
  const last = Number(stateLast) || 0;
  if (last > 0) return last;
  let newest = 0;
  for (const t of rows || []) {
    if (t?.kind === "substack") continue;
    const sec = Math.floor(new Date(t.time).getTime() / 1000);
...
export async function runIngest({
  ...
  let stopped = null;
  let xSuccess = false;
  let xSkipped = apiKey ? null : "no_key";
  // Billing/id cache is separate from portal KEEP. Skip only ids already in live.json.
  const liveIds = new Set(existingRows.map((t) => String(t.id)));
  const seen = new Set([...(state.seenIds || []), ...liveIds]);

  if (apiKey && state.credits.used < budget) {
    const nowSec = Math.floor(now.getTime() / 1000);
    const hasLiveFeed = existingRows.some((t) => t.kind !== "substack");
    const lastIngest = lastIngestFromLive(existingRows, state.lastIngestUnix);
    const plan = lookbackPlan(nowSec, lastIngest, { hasLiveFeed });
    const since = plan.since;
    const maxPages = plan.catchup ? CATCHUP_PAGES_PER_BATCH : STEADY_PAGES_PER_BATCH;
    ...
    for (const batch of batches) {
      ...
      for (let page = 0; page < maxPages; page++) {
        ...
        for (const tweet of tweets) {
          ...
          if (!idCache.has(id)) idCache.set(id, tweet);
          // Already on the portal. Do not skip merely because the billing cache saw the id.
          if (liveIds.has(id)) {
            dropped += 1;
            continue;
          }
    ...
    if (xSuccess) state.lastIngestUnix = nowSec;
  } else if (apiKey) {
    xSkipped = "daily_cap";
  }

  let substack = { ok: false, kept: [], fetched: 0, dropped: 0, feeds: 0 };
  if (!skipSubstack) {
    substack = await ingestSubstack({ now, fetchImpl, universe, seen });
    kept.push(...substack.kept);
    fetched += substack.fetched;
    dropped += substack.dropped;
  }

  const anySuccess = xSuccess || substack.ok;
  ...
  return {
    ...
    budget,
    wrote,
    x: { skipped: xSkipped, kept: kept.filter((r) => r.kind !== "substack").length },
    substack: { feeds: substack.feeds, kept: substack.kept.length, fetched: substack.fetched },
  };
}
