import { categoryForTickers, findSectorHits, findTickers } from "./universe.js";
...
const PROMO = /\b(subscribe now|become a paid subscriber|upgrade to paid|leave a comment)\b/i;
const HOWTO = /\b(how to (?:prompt|use chatgpt|get started)|tutorial for beginners|tips and tricks)\b/i;
const POLITICS = /\b(election|democrat|republican|kamala|congress vote|party primary)\b/i;

/**
 * Substack KEEP: two concrete numbers + universe name or sector/theme.
 * DROP thin/promo/how-to/politics with no sector hit.
 */
export function judgeSubstackKeep(corpus, { universe } = {}) {
  const text = String(corpus || "");
  if (!text.trim()) return { keep: false, reason: "empty" };
  if (DROP_MEME.test(text) || PRICE_SPAM.test(text)) return { keep: false, reason: "spam" };

  const tickers = universe ? findTickers(text, universe) : [];
  const themes = themesFromText(text);
  const sectorHits = universe ? findSectorHits(text, universe) : [];
  const numbers = concreteNumbers(text);
  const tagged = tickers.length > 0 || themes.length > 0 || sectorHits.length > 0;
  const topic = TOPIC.test(text) || tagged;
  if (!topic) return { keep: false, reason: "off-topic" };
  if (POLITICS.test(text) && !tagged) return { keep: false, reason: "politics" };
  if (HOWTO.test(text) && !tickers.length) return { keep: false, reason: "how-to" };
  const stripped = text.replace(/https?:\/\/\S+/g, "").trim();
  if (PROMO.test(text) && stripped.length < 240 && numbers.length < 2) {
    return { keep: false, reason: "promo" };
  }
  if (stripped.length < 80) return { keep: false, reason: "thin" };
  if (numbers.length < 2) return { keep: false, reason: "no-evidence" };
  if (!tagged) return { keep: false, reason: "no-sector" };

  return { keep: true, tickers, themes, numbers, sectorHits };
}

export function toSubstackRow(post, { corpus, media, universe, pub }) {
  const judged = judgeSubstackKeep(corpus, { universe });
  if (!judged.keep) return { keep: false, reason: judged.reason };
  const tickers = judged.tickers || [];
  let themes = judged.themes || [];
  let category = categoryForTickers(tickers);
  ...
  if (universe) {
    ...
    for (const s of judged.sectorHits || []) {
      if (universe.sectors?.has(s) && !sectors.includes(s)) sectors.push(s);
    }
  }
  const summary = oneLineSummary(post.title || corpus);
  if (!summary) return { keep: false, reason: "empty-summary" };
  const full = String(corpus || post.title || "").slice(0, 8000);
  return {
    keep: true,
    item: {
      id: post.id,
      time: parseTweetDate(post.pubDate).toISOString(),
      summary,
      full_text: full,
      handle: pub.handle,
      source: pub.source || pub.name,
      category,
